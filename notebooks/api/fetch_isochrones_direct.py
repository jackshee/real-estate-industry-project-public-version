...
def fetch_single_isochrone(
    ...
    for attempt in range(max_retries):
        try:
            logger.debug(f"Fetching isochrone for coordinates: {coordinates} (attempt {attempt + 1})")
...
def fetch_batch_isochrones(
    ...
    for attempt in range(max_retries):
        try:
            logger.debug(f"Fetching isochrones for {len(coordinates_list)} locations (attempt {attempt + 1})")
            ...
            logger.error(f"Error fetching batch isochrones (attempt {attempt + 1}): {e}")
            if attempt < max_retries - 1:
                wait_time = 2 ** attempt
                logger.info(f"Retrying in {wait_time} seconds...")
                time.sleep(wait_time)
...
def process_multiple_listings(
    ...
    for batch_start in range(0, total_listings, batch_size):
        ...
        logger.info(f"Processing batch {batch_start//batch_size + 1}: listings {batch_start+1}-{batch_end}")
        ...
        logger.info(f"Extracted coordinates for {len(coordinates_list)} listings in this batch")
        ...
        if isochrone_data and isochrone_data.get('features'):
            logger.info(f"Successfully fetched batch isochrone data! (Request #{total_requests})")
            logger.info(f"Number of features: {len(isochrone_data.get('features', []))}")
            ...
            logger.error(f"Failed to fetch batch isochrone data for batch {batch_start//batch_size + 1}")
        ...
        if batch_start + batch_size < total_listings:  # Don't delay after the last batch
            time.sleep(0.5)  # 500ms delay between batches

    # Consolidate all data (successful and failed)
    logger.info(f"Consolidating data from {successful_batches} successful batches and {failed_batches} failed batches")
    ...
    output_path = Path(output_dir) / f"isochrone_{file_number}.csv"
    save_isochrone_data(combined_isochrone_data, str(output_path), final_combined_listings)
    
    logger.info(f"Total API requests made: {total_requests}")
    logger.info(f"Successfully processed {len(combined_features)} isochrone features from {successful_batches} successful batches")
    logger.info(f"Failed batches: {failed_batches}")
    logger.info(f"Output file will have {len(final_combined_listings)} rows (same as input)")
