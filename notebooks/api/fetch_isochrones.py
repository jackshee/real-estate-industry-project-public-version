#!/usr/bin/env python3
"""
Isochrone fetching script for rental listings data.

This script processes missing isochrones data by fetching driving and walking
isochrones using the OpenRouteService API. It takes API keys and a path to
missing isochrones data as command line arguments.

Usage:
    # Using command line arguments
    python fetch_isochrones.py --api-key1 <key1> --api-key2 <key2> --input-path <path>

    # Using environment variables (recommended)
    python fetch_isochrones.py --input-path <path>
"""
...
def update_main_dataset(gdf_processed, main_dataset_path):
    """
    Update the main dataset with processed isochrone data.
    Only merges rows that have non-null isochrone data.

    Args:
        gdf_processed (geopandas.GeoDataFrame): Processed data with isochrones
    ...
    try:
        ...
        logger.info(
            f"Number of missing coordinates before imputation: {df_main['driving_5min'].isnull().sum()}"
        )

        # Select only the columns we need from processed data for the merge
        isochrone_cols = [
            "property_id",
            "driving_5min",
            "driving_10min",
        ...
        gdf_subset = gdf_processed[isochrone_cols].copy()

        # Filter to only include rows that have at least one non-null isochrone value
        # Check if any of the isochrone columns have non-null values
        has_isochrone_data = (
        ...
        # Only keep rows with at least one non-null isochrone value
        gdf_subset_filtered = gdf_subset[has_isochrone_data].copy()

        logger.info(
            f"Filtered to {len(gdf_subset_filtered)} rows with non-null isochrone data out of {len(gdf_subset)} total processed rows"
        )

        if len(gdf_subset_filtered) == 0:
            logger.warning(
                "No rows with valid isochrone data to merge. Skipping dataset update."
            )
            return
        ...
        logger.info(
            f"Number of missing coordinates after imputation: {df_updated['driving_5min'].isnull().sum()}"
        )
...
def update_missing_isochrones_file(input_path, gdf_processed):
    ...
    try:
        ...
        # Find which property_ids were successfully processed (have at least one non-null isochrone)
        has_isochrone_data = (
        ...
        successfully_processed_ids = set(
            gdf_processed[has_isochrone_data]["property_id"].tolist()
        )
...
def main():
    """Main function to orchestrate the isochrone fetching process."""
    parser = argparse.ArgumentParser(
        description="Fetch isochrones for missing rental listings data",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Using environment variables (recommended - set ORS_API_KEY1 and ORS_API_KEY2 in .env)
  python fetch_isochrones.py --input-path ../data/raw/missing_isochrones_0.csv
  
  # Using command line arguments
  python fetch_isochrones.py --api-key1 abc123 --api-key2 def456 --input-path ../data/raw/missing_isochrones_0.csv
  
  # With custom main dataset and testing with limited records
  python fetch_isochrones.py --input-path ../data/raw/missing_isochrones_0.csv --max-records 50
        """,
    )

    parser.add_argument(
        "--api-key1",
        help="First OpenRouteService API key for driving isochrones (or set ORS_API_KEY1 env var)",
    )
    parser.add_argument(
        "--api-key2",
        help="Second OpenRouteService API key for walking isochrones (or set ORS_API_KEY2 env var)",
    )
    parser.add_argument(
    ...
    api_key1 = args.api_key1 or os.getenv("ORS_API_KEY1")
    api_key2 = args.api_key2 or os.getenv("ORS_API_KEY2")

    # Validate API keys are provided
    if not api_key1:
        logger.error(
            "API key 1 not provided. Use --api-key1 or set ORS_API_KEY1 environment variable"
        )
        sys.exit(1)
    if not api_key2:
        logger.error(
            "API key 2 not provided. Use --api-key2 or set ORS_API_KEY2 environment variable"
        )
        sys.exit(1)
    ...
    try:
        ...
        # Initialize GeoUtils instances
        logger.info("Initializing GeoUtils instances...")
        geoutils_driving = GeoUtils(ors_api_key=api_key1)
        geoutils_walking = GeoUtils(ors_api_key=api_key2)
        ...
        logger.info("Isochrone fetching completed successfully!")
